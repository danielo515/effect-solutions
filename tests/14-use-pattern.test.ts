...
describe("14-use-pattern", () => {
  ...
  class FileSystem extends Effect.Service<FileSystem>()("FileSystem", {
    effect: Effect.gen(function* () {
      ...
      const use = <A>(
        fn: (fs: MockFs, signal: AbortSignal) => Promise<A>
    ...
    it.effect("wraps Promise-based APIs in Effect", () =>
      pipe(
        Effect.gen(function* () {
          ...
          const content = yield* fileSystem.use((fs) =>
    ...
    it.effect("supports write operations", () =>
      pipe(
        Effect.gen(function* () {
          ...
          yield* fileSystem.use((fs) =>
            fs.writeFile("new-file.txt", "new content")
          );

          const content = yield* fileSystem.use((fs) =>
            fs.readFile("new-file.txt")
          );
