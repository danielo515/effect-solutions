...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    ...
    h2: ({ children, className, ...props }) => (
      <h2
        className={cn(
          "text-[1.05rem] font-semibold leading-snug text-neutral-100 mx-6 uppercase",
          className,
        )}
        {...props}
      >
    ...
    h3: ({ children, className, ...props }) => (
      <h3
        className={cn(
          "text-2xl font-medium leading-snug text-neutral-200 mx-6",
          className,
        )}
        {...props}
      >
    ...
    sup: ({ children }: { children: ReactNode }) => {
      ...
      if (childArray.length === 1) {
        ...
        if (isValidElement(child) && child.props["data-footnote-ref"]) {
          const anchorId =
